...
import { Clock, MapPin, Moon, Book, Compass, Sun, Settings } from 'lucide-react-native';
...
export default function HomeScreen() {
  ...
  const QuickAccessCard = ({ title, icon, onPress, theme }) => (
    ...
    >
      {icon}
      <Text style={[styles.quickAccessText, { color: theme.text.primary }]}>{title}</Text>
    </TouchableOpacity>
  );
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.background }]}>
      ...
      >
        ...
        <View style={[styles.nextPrayerContainer, { backgroundColor: theme.primary }]}>
          <Text style={[styles.nextPrayerTitle, { color: theme.background }]}>{t('nextPrayer')}</Text>
          <Text style={[styles.nextPrayerName, { color: theme.background }]}>{nextPrayer}</Text>
          <Text style={[styles.nextPrayerTime, { color: theme.background }]}>{t('remainingTime')}: {remainingTime}</Text>
        </View>
          ...
          <View style={styles.ramadanContainer}>
            <View style={[styles.ramadanCard, { 
              ...
              <View style={styles.ramadanIconContainer}>
                ...
                <Text style={[styles.ramadanTitle, { color: theme.text.primary }]}>{t('timeToIftar')}</Text>
              </View>
              <Text style={[styles.ramadanTime, { color: theme.primary }]}>{iftarTime}</Text>
              <Text style={[styles.ramadanInfo, { color: theme.text.secondary }]}>{t('iftar')}: {prayerTimes?.Maghrib}</Text>
            </View>
            ...
            <View style={[styles.ramadanCard, { 
              ...
              <View style={styles.ramadanIconContainer}>
                ...
                <Text style={[styles.ramadanTitle, { color: theme.text.primary }]}>{t('timeToSahur')}</Text>
              </View>
              <Text style={[styles.ramadanTime, { color: theme.primary }]}>{sahurTime}</Text>
              <Text style={[styles.ramadanInfo, { color: theme.text.secondary }]}>{t('sahur')}: {prayerTimes?.Fajr}</Text>
            </View>
          </View>
        )}

        <View style={styles.quickAccessContainer}>
          <Text style={[styles.sectionTitle, { color: theme.text.primary }]}>{t('quickAccess')}</Text>
        ...
        {prayerTimes && (
          <View style={[styles.todayTimesContainer, { backgroundColor: theme.surface }]}>
            <Text style={[styles.sectionTitle, { color: theme.text.primary }]}>{t('todayTimes')}</Text>
            <View style={[styles.timesList, { backgroundColor: theme.card.background }]}>
              <View style={[styles.timeItem, { borderBottomColor: theme.border }]}>
                <Text style={[styles.timeLabel, { color: theme.text.primary }]}>{t('fajr')}</Text>
                <Text style={[styles.timeValue, { color: theme.prayer.fajr }]}>{prayerTimes.Fajr}</Text>
              </View>
              <View style={[styles.timeItem, { borderBottomColor: theme.border }]}>
                <Text style={[styles.timeLabel, { color: theme.text.primary }]}>{t('sunrise')}</Text>
                <Text style={[styles.timeValue, { color: theme.prayer.sunrise }]}>{prayerTimes.Sunrise}</Text>
              </View>
              <View style={[styles.timeItem, { borderBottomColor: theme.border }]}>
                <Text style={[styles.timeLabel, { color: theme.text.primary }]}>{t('dhuhr')}</Text>
                <Text style={[styles.timeValue, { color: theme.prayer.dhuhr }]}>{prayerTimes.Dhuhr}</Text>
              </View>
              <View style={[styles.timeItem, { borderBottomColor: theme.border }]}>
                <Text style={[styles.timeLabel, { color: theme.text.primary }]}>{t('asr')}</Text>
                <Text style={[styles.timeValue, { color: theme.prayer.asr }]}>{prayerTimes.Asr}</Text>
              </View>
              <View style={[styles.timeItem, { borderBottomColor: theme.border }]}>
                <Text style={[styles.timeLabel, { color: theme.text.primary }]}>{t('maghrib')}</Text>
                <Text style={[styles.timeValue, { color: theme.prayer.maghrib }]}>{prayerTimes.Maghrib}</Text>
              </View>
              <View style={[styles.timeItem, { borderBottomColor: theme.border }]}>
                <Text style={[styles.timeLabel, { color: theme.text.primary }]}>{t('isha')}</Text>
                <Text style={[styles.timeValue, { color: theme.prayer.isha }]}>{prayerTimes.Isha}</Text>
              </View>
            </View>
          </View>
        )}
